...
public class ReplicateImageModel implements ImageModel {
    ...
    public ImageResponse call(ImagePrompt request) {
        ...
        // Replicate 요청 객체 생성
        ReplicateDto.ReplicateInput input = new ReplicateDto.ReplicateInput(
                request.getInstructions().get(0).getText(), // 프롬프트
                aspectRatio, // 이미지 비율
        ...
        ReplicateDto.ReplicateRequest replicateRequest = new ReplicateDto.ReplicateRequest(input);

        log.info("Replicate Flux.1 이미지 생성 요청. Prompt: {}", finalPrompt);

        // API 호출 (동기 방식)
        ReplicateDto.ReplicateResponse response = restClient.post()
                .uri("/models/black-forest-labs/flux-schnell/predictions") // 모델명으로 API 호출
                .header("Authorization", "Bearer " + apiToken)
                .header("Prefer", "wait") // 생성 완료까지 대기
                .contentType(MediaType.APPLICATION_JSON)
                .body(replicateRequest)
                .retrieve()
                .body(ReplicateDto.ReplicateResponse.class);
        ...
        List<ImageGeneration> generations = response.output().stream()
